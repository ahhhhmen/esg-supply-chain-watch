...
- **终端个案交通事故与民事诉讼物理阻断规则 (Retail Accident & Personal Lawsuit Exclusion)**：终端车企/经销商发生的个案交通事故、单车火灾/起火/爆炸事故、局部消防救援（未伴随官方监管机构正式立案缺陷调查或批量召回）、试驾碰撞/索赔、个人人身伤害索赔诉讼、买卖/定金/维权纠纷，绝对不属于 ESG 供应链风险。LLM System Prompt 与 Python 侧 `_apply_materiality_guardrails` (`_RETAIL_ACCIDENT_CLAIM_RE`) 均强制判定为 `is_valid_risk = False` 直接物理丢弃。
- **去重记忆库精准缓存策略 (Precision URL Caching Protocol)**：`sourcing_engine.py` 在 Phase 1 原始抓取阶段绝对禁止盲目将未校验的 raw URLs 全量写入 `logs/processed_urls.json`；只有在 Phase 4/5 经过实体校验与限流过滤、真正进入分析管道的 URL，才由 `engine.save_processed_urls()` 增量保存，防止 raw 链接过拦截导致后续跑批盲区与推送断流。
