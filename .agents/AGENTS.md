...
- **终端个案交通事故与民事诉讼物理阻断规则 (Retail Accident & Personal Lawsuit Exclusion)**：终端车企/经销商发生的个案交通事故、试驾碰撞/索赔、个人人身伤害索赔诉讼、买卖/定金/维权纠纷，绝对不属于 ESG 供应链风险。LLM System Prompt 与 Python 侧 `_apply_materiality_guardrails` (`_RETAIL_ACCIDENT_CLAIM_RE`) 均强制判定为 `is_valid_risk = False` 直接物理丢弃。
