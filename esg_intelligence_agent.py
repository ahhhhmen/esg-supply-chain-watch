...
2. 降噪判定 (is_valid_risk) — 宁可保留早期信号，不可漏过潜在风险：
   - 若实体错误（如"福特省长"、"福特医院"当成福特汽车，或发生安全事故的物理主体非监控车企自身而只是非核心零部件供应商），值为 false。
   ...
   - 以下可判定为 false：纯股价涨跌、财报亏损、CEO言论、基金调研、券商研报、M&A传闻（未证实）、产品参数披露、技术路线探讨、终端车企/经销商发生的个案交通事故、试驾事故/碰撞、个案单车火灾/起火/爆炸/消防救援（未伴随官方监管立案缺陷调查或批量召回）、个案人身伤害索赔诉讼、个案退定金/加价/售后纠纷、车辆盗窃或维修个案（绝对不属于 ESG 供应链风险，必须判定 is_valid_risk=false）。
...
3. 严格分类 (risk_category) 与负面清单：
   ...
   - "合规与运营危机"：包含劳工罢工/抗议、重大安全事故（尾矿坝溃坝/爆炸/矿难）、产品召回、批量车辆起火/缺陷调查、软件安全缺陷、严重环保罚单。【负面清单】绝对排除终端个案交通事故、单车火灾/爆炸/碰撞、试驾碰撞/索赔、个人民事伤害诉讼、买卖合同/定金纠纷——这些归入 is_valid_risk=false 并直接丢弃。
...
4. 材料冲击判定 (is_direct_material_impact) — 绝对红线：
   ...
   · 主机厂常规厂区/非电池制造车间发生的化学品泄漏、普通环保安全事故、或非电池核心零部件污染 → false。理由：非电池主轴物料危机，不触发上游前驱体/正极材料订单穿透。
...
11. 【历史基线对照约束】：涉及政府公报、国际倡议（如 G7 声明）或国家税制法案时，历史基线对比必须基于真实的历史政策。绝对禁止将第三方 ESG 行业认证标准（如 IRMA/RMI 等）的文案错配到国际政治声明或政府关税税制上。若无确切历史对比，应如实说明系统暂未收录。
12. 【官方寻源域名防错】：新闻媒体报道域名（如 `bjnews.com.cn`, `reuters.com`, `bloomberg.com`, `sina.com.cn` 等）绝对禁止识别为政府官方机构域名（如中国商务部官网 `mofcom.gov.cn`）。只有顶级政府/官方域名才可标为官方机构域名。
...
3. 具体切入点（至少覆盖以下一个维度）：
   ...
   - 披露体系：对方的 ESG 报告框架、数据收集方法是否可被华友借鉴
	...
	6. 语言质量硬约束：输出的分析文本必须符合资深商业顾问的行文规范。请在输出前进行严格的自我语病审查，确保语言精炼、专业、流畅，绝对禁止出现词句重复或语法结构杂糅。
	7. 正例：
...
- **语言质量硬约束**：输出需符合资深商业顾问行文规范，输出前进行自我语病审查，确保语言精炼、专业、流畅，禁止词句重复、语法结构杂糅或逻辑自相矛盾。"""
    ...
    _RETAIL_ACCIDENT_CLAIM_RE = re.compile(
        ...
        r"(在.*)?(发生爆炸|车辆爆炸|单车起火|单车爆炸|爆炸导致消防员|消防员受伤|消防员送医|局部火灾)|"
        ...
        r"vehicle explosion|car explosion|explosion in .* (injured|firefighter)|firefighter injured in explosion",
        re.IGNORECASE,
    )
    ...
    def _format_for_dingtalk(cls, valid_events: list[dict], mode: str, now_str: str, dmin: str, dmax: str, weekly_review_text: str = None) -> str:
        ...
        material_events = [
            ...
            if e.get("materiality") != "🟡 战略观察" and e.get("is_direct_material_impact", True) is not False
        ]
        ding_watch = [
            e for e in valid_events
            if e.get("materiality") == "🟡 战略观察" or e.get("is_direct_material_impact", True) is False
        ]
        ...
        for cat in cls.CATEGORY_ORDER:
            ...
            for e in evs:
                ...
                insight = str(e.get("executive_insight", "")).strip()
                date = str(e.get("date", ""))[:10]
                is_material = bool(e.get("is_direct_material_impact", True))
                ...
                lines.append(f"> ⚖️ **影响分级**：{e.get('materiality', '🔴 直接材料冲击')}")
                lines.append("> ")
        ...
        if mode == "weekly" and weekly_review_text:
            ...
            lines.append("")
            for br_line in str(weekly_review_text).splitlines():
